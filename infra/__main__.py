...
Manages: API droplets, DNS records (Cloudflare + DO), VPC networking, firewalls.
Stacks: dev, prod (each stack is fully isolated).

SSL strategy:
  - API (DNS-only, no CF proxy): Let's Encrypt cert via DNS-01 challenge through Cloudflare
  - Portal (CF-proxied): Cloudflare origin cert (CF handles public SSL)
...
# VPC
vpc_id = config.get("vpc_id") or "159def95-d05a-4ab9-9618-b670ceada0bb"
...
api_firewall = do.Firewall(
    ...
    inbound_rules=[
        ...
        do.FirewallInboundRuleArgs(
            ...
            source_addresses=["10.108.0.0/20"]),
    ],
    outbound_rules=[
